...
Implement tasks from an OpenSpec change using an advanced supervisor agent that orchestrates specialized sub-agents (Test, Make, Review) for a robust, TDD-driven development cycle.
...
4. **Sub-Task Implementation Loop**
   Work on each task in the todo list separately using the following loop.
