...
### D3 — Dynamic fields are converted to `ValueExprDefinition` at project load

New step in `Project.from_definition` (runs after entity construction and trait `post_init` hooks): scan every entity's declared model fields **and** extra attributes; any value that is a `str` starting with `=` is parsed (full-match, D1) and replaced with a `ValueExprDefinition` instance holding the AST and the source string. Parse failure → `ValueError` naming the entity, field, and parse error — project load fails. Fields backed by setter-less properties (e.g. `current_node_id` on graph entities) are rejected with a clear error.
...
**Implementation note (deviation)**: the original plan ran this in a `BaseEntity.post_init` hook. In practice `entity.py` cannot import the expression schema: `conditions/__init__` pulled in the evaluator, and the evaluator imports `schemas.entity` (import cycle), while trait `post_init` overrides do not chain to a base hook. Resolution: `conditions/__init__` now re-exports the grammar only (the evaluator stays importable as `llm_gamebook.story.conditions.evaluator`), and the conversion + cycle detection live in `Project.from_definition` (see `iter_entity_field_values` / `convert_dynamic_fields` / `detect_dynamic_field_cycles` in `schemas/project.py`).
