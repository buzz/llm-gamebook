...
Condition-based automatic action dispatch after agent steps. Triggers are defined on entity types in YAML (action name, boolean condition, args), evaluated against the effective state after each action's state changes are committed, and dispatch their configured action when the condition is true.
