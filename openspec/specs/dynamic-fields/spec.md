...
Entity fields whose value is a string prefixed with `=` are dynamic fields: expressions parsed at project load and evaluated at access time against the current effective state, providing derived, read-only values (computed stats, flags, chains) that stay in sync with session state.
