...
Player-facing controls for session state history: a "Game" toolbar group in the player view with Undo, state history browsing, Fork, End game, and Reset; an ended-session banner that locks input; and the `maxStateHistory` setting in the settings form.
