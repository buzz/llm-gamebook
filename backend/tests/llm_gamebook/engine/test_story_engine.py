...
type StreamRunnerEvents = list[ResponseStreamUpdateMessage]


@pytest.fixture
def stream_runner_events(message_bus: MessageBus) -> StreamRunnerEvents:
    events: StreamRunnerEvents = []
...
async def test_stream_runner_run_returns_messages(
    ...
    stream_runner_events: StreamRunnerEvents,
) -> None:
...
async def test_stream_runner_handles_model_request_node(
    ...
    assert len(stream_runner_events) >= 1
    event = stream_runner_events[0]
...
async def test_stream_runner_handles_part_delta_event(
    ...
    assert len(stream_runner_events) >= 1
    for event in stream_runner_events:
        assert event.session_id == stream_runner._session_id
...
async def test_stream_runner_response_contains_model_output(
    ...
    assert len(stream_runner_events) >= 1
    final_event = stream_runner_events[-1]
