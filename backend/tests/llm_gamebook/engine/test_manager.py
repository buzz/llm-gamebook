...
async def test_engine_manager_get_or_create_returns_same_engine(
    ...
    engine1 = await engine_manager.get_or_create(session.id, db_session, project_manager)
    engine2 = await engine_manager.get_or_create(session.id, db_session, project_manager)
...
async def test_engine_manager_create_model_and_state(
    ...
    model, context = await engine_manager._create_model_and_context(
...
async def test_engine_manager_create_model_and_state_missing_session(
    db_session: AsyncDbSession, project_manager: ProjectManager, engine_manager: EngineManager
) -> None:
    non_existent_session_id = uuid4()

    with pytest.raises(ValueError, match=r"Session .* not found"):
        await engine_manager._create_model_and_context(
            non_existent_session_id, db_session, project_manager
        )
